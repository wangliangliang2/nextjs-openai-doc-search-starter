...
**imageMogr2** 提供一系列高级图片处理功能，本文介绍 **去除图片元信息**，可与 **imageMogr2** 的其他参数一起使用，详细的计费说明请参见 [计费与定价](https://www.qiniu.com/prices/dora?source=dora)。
...
| 参数名称                      | 必填 | 说明                                                         |
| ----------------------------- | ---- | ------------------------------------------------------------ |
| `/strip/<strip>`              |      | 去除图片中的元信息。去除的信息有：bKGD、cHRM、EXIF、gAMA、iCCP、iTXt、sRGB、tEXt、zCCP、zTXt、date |
| `/ignore-error/<ignoreError>` |      | 取值为 1 时，则处理失败时返回原图；取值为 0 时，则处理失败时返回错误信息。默认值为 0。 |

# **示例**

```
https://dn-odum9helk.qbox.me/resource/gogopher.jpg?imageMogr2/strip
```
