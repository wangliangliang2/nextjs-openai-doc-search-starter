...
**imageMogr2** 提供一系列高级图片处理功能，本文介绍 **图像DPI**，即输出分辨率主要指印刷需要的分辨率，描述的是设备输出图像时每英寸可产生的点数(dpi)。

可与 **imageMogr2** 的其他参数一起使用，详细的计费说明请参见 [计费与定价](https://www.qiniu.com/prices/dora?source=dora)。
...
- 原图限制
  - 原图格式支持： `psd`、`jpeg`、`png`、`gif`、`webp`、`tiff`、`bmp`（webp动图，仅支持持久化处理）
- 输出限制
...
- 设置输出分辨率为300DPI，进行处理。
  ...
  ```
  https://dn-odum9helk.qbox.me/resource/gogopher.jpg?imageMogr2/density/300
  ```

  ![img](https://dn-odum9helk.qbox.me/resource/gogopher.jpg?imageMogr2/density/300)
