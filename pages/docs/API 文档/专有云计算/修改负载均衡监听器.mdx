...
# 请求路径
> `PUT` /api/rio/v1/network/loadbalancer_listener/:listener_id
...
listener_id             | string | path | 否 | 监听器 ID
enabled                 | boolean | body | 否 | 启用状态：<ul><li>`true` : 启用</li><li>`false` : 停用</li></ul> 
listener_name           | string | body | 否 | 名称
description             | string | body | 否 | 描述
protocol                | string | body | 否 | 监听协议：<ul><li>`tcp`</li><li>`udp`</li><li>`https` :tcp 转发实现</li><li>`http`</li><li>`sctp`</li><li>`terminated_https` : https 协议，创建时需绑定证书</li></ul> 
listener_port           | integer | body | 否 | 负载均衡实例前端使用的端口。取值：1-65535。
connection_limit        | integer | body | 否 | 最大连接数，默认值为 -1 ，表示无限连接
timeout_client_data     | integer | body | 否 | 前端客户端不活动超时，单位：秒，默认值：50
timeout_member_connect  | integer | body | 否 | 后端成员连接超时，单位：秒，默认值：5
timeout_member_data     | integer | body | 否 | 后端成员不活动超时，单位：秒，默认值：50
timeout_tcp_inspect     | integer | body | 否 | 等待 TCP 数据包检查超时，单位：秒，默认值：0
persistence_timeout     | integer | body | 否 | 会话保持的超时时间。取值：0-3600（秒）。默认值：0，表示关闭会话保持。
established_timeout     | integer | body | 否 | 连接超时时间。取值：10-900（秒）。 默认值: 900
insert_headers          | []string | body | 否 | 请求发送到后端服务器时添加的请求头：<ul><li>`X-Forwarded-For` : 记录最初发起请求的客户端 IP</li><li>`X-Forwarded-Port` : 记录最初发起请求的客户端 Port</li><li>`X-Forwarded-Proto` :记录负载均衡服务器与后端客户端之间采用的协议类型</li></ul> 
default_pool_id         | string | body | 否 | 资源池 ID
...
```json
{
    "listener_name":"slb_listener_modify"
}
```
