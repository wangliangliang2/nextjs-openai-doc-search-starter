将时间格式转换为时间戳
![](https://dn-odum9helk.qbox.me/FiZ_y_6W1XE7jB0ALQL6pW7tw87u)

* 要进行Transform变化的键(`key`): 支持多级嵌套，多级间用 *.* 符号连接，如 abc.xyz，表示 data["abc"]["xyz"]中的数据。

* 新的字段名(`new`): 选填，新的字段名称。

* 精度(`precision`): 默认为 seconds，可以选择milliseconds, microseconds, nanoseconds，分别代表毫秒，微秒，纳秒。
...
* 要进行Transform变化的键已存在时，是否覆盖原有的值(`override`): 默认为 `false`，为 `true` 时会覆盖原有值。
