...
连麦服务端 SDK 已经整合到直播服务端 SDK 中，目前提供的语言有 Java,PHP,Python
...
使用七牛颁发的AccessKey和SecretKey，对本次http请求的信息进行签名，生成管理凭证。签名的原始数据包括http请求的Method, Path, RawQuery, Content-Type及Body等信息，这些信息的获取方法取决于具体所用的编程语言，建议参照七牛提供的SDK代码。
...
OwnerUserId: 要创建房间的所有者，最大长度50，需满足规格^[a-zA-Z0-9_-]{3,50}$ ,string类型

RoomName: 要创建的房间名称，可选，最大长度64位，需满足规格^[a-zA-Z0-9_-]{3,64}$,string类型。如果没有指定，系统返回一个uuid作为房间名称。
...
UserId: 当前在线的连麦用户的id，是区分用户的唯一标志

UserName: 当前在线的连麦用户的昵称
...
## 7.1 请求包
    ...
    DELETE /v1/rooms/<RoomName>/users/<UserId>
    Host: rtc.qiniuapi.com 
    Authorization: <QiniuToken> 
