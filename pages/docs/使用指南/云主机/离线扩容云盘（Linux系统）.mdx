...
| 资源 | 限制条件 |
|-|-|
| 实例 |实例状态为运行中或者已停止。<br>实例的Linux内核不低于3.6.0版本。您可以使用uname -a命令查看内核版本。<br>如果Linux内核低于3.6.0版本，扩容分区操作请参见扩展低内核版本实例的系统盘分区和文件系统和扩展分区和文件系统_Linux数据盘。<br><br>**说明**：如果您的QVM实例不符合在线扩容条件，您可以使用离线扩容云盘功能. |
