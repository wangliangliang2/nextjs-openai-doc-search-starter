...
|操作系统|程序名|地址|  
|---|-----|---|  
|windows 32 位|qsuits_windows_386.exe|[下载](https://devtools.qiniu.com/qsuits_windows_386.exe)|  
|windows 64 位|qsuits_windows_amd64.exe|[下载](https://devtools.qiniu.com/qsuits_windows_amd64.exe)|  
|linux 32 位|qsuits_linux_386|[下载](https://devtools.qiniu.com/qsuits_linux_386)|  
|linux 64 位|qsuits_linux_amd64|[下载](https://devtools.qiniu.com/qsuits_linux_amd64)|  
|mac(64 位)|qsuits_darwin_amd64|[下载](https://devtools.qiniu.com/qsuits_darwin_amd64)|  
...
![](http://devtools.qiniu.com/windows_user_path.png)  
...
![](http://devtools.qiniu.com/choose_install_java_message.jpg)  
...
# 运行模式
### （1）批处理模式：[读取[数据源](#8)] => [选择[过滤器](#9)] => [数据[处理过程](#10)] => [[结果持久化](#11)]  
...
|处理类型配置          |含义                                         |文档链接                                    |  
|--------------------|--------------------------------------------|-------------------------------------------|  
|process=qupload      | 上传文件到存储空间                        | [qupload 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/uploadfile.md)         |  
|process=syncupload   | 将 url 的内容直传到存储空间                | [syncupload 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/syncupload.md)      |  
|process=delete       | 删除空间资源                             | [delete 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/delete.md)              |  
|process=copy         | 复制资源到指定空间                        | [copy 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/copy.md)                  |  
|process=move         | 移动资源到指定空间                        | [move 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/move.md)                  |  
|process=rename       | 对指定空间的资源进行重命名                 | [rename 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/rename.md)              |  
|process=stat         | 查询空间资源的元信息                      | [stat 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/stat.md)                  |  
|process=type         | 修改空间资源的存储类型（低频/标准/归档）     | [type 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/type.md)                  |  
|process=restorear    | 对空间的归档文件进行解冻                   | [restorear 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/restorear.md)        |  
|process=status       | 修改空间资源的状态（启用/禁用）             | [status 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/status.md)              |  
|process=lifecycle    | 修改空间资源的生命周期                     | [lifecycle 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/lifecycle.md)        |  
|process=mirror       | 对设置了镜像源的空间资源进行镜像更新         | [mirror 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/mirror.md)              |  
|process=asyncfetch   | 异步抓取 url 资源到指定空间                | [asyncfetch 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/asyncfetch.md)      |  
|process=fetch        | 同步抓取 url 资源到指定空间                | [fetch 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/fetch.md)                |  
|process=qhash        | 查询资源的 qhash                         | [qhash 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/qhash.md)                |  
|process=avinfo       | 查询空间资源的视频元信息                    | [avinfo 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/avinfo.md)             |
|process=pfopcmd      | 根据音视频资源的 avinfo 信息来生成转码指令   | [pfopcmd 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/pfopcmd.md)           |  
|process=pfop         | 对空间资源执行 pfop 请求                   | [pfop 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/pfop.md)                 |  
|process=pfopresult   | 通过 persistentId 查询 pfop 的结果        | [pfopresult 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/pfopresult.md)     |  
|process=privateurl   | 对私有空间资源进行私有签名                  | [privateurl 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/privateurl.md)     |  
|process=publicurl    | 对资源生成 url，导出公开 url 或添加参数      | [publicurl 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/publicurl.md)       |  
|process=exportts     | 对 m3u8 的资源进行读取导出其中的 ts 文件列表  | [exportts 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/exportts.md)        |  
|process=download     | 通过 http 下载资源到本地                   | [download 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/downloadfile.md)     |  
|process=imagecensor  | 图片类型资源内容审核                       | [imagecensor 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/censor.md#图片审核) |  
|process=videocensor  | 视频类型资源内容审核                       | [videocensor 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/censor.md#视频审核) |  
|process=censorresult | 内容审核结果查询                          | [censorresult 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/censorresult.md)  |  
...
|process=refreshquery | 查询 CDN 刷新结果                         | [refreshquery 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/cdn.md#cdn-刷新预取结果查询) |  
|process=prefetchquery| 查询 CDN 预热结果                         | [prefetchquery 配置](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/cdn.md#cdn-刷新预取结果查询) |  
...
```
程序会自动重试，如果比较频繁则可以修改[超时配置](#12)重新运行程序，超过重试次数或者其他非预期异常发生时程序会退出，可以将异常信息反馈在[ ISSUE 列表](https://github.com/NigelWu95/qiniu-suits-java/issues) 中。  

2、常见错误信息：  
（1）java.lang.UnsupportedClassVersionError: Unsupported major.minor version ...  
请使用 java 8 或以上版本的 jdk（jre） 环境来运行该程序。  
（2）java.lang.OutOfMemoryError: GC overhead limit exceeded  
表示可能是内存中加载了过多的资源导致 java 的 gc 内存溢出，需要关闭程序重新运行，更换高配置机器或者降低线程数 threads 或者 unit-len 重新运行，如果已运行较长时间可采用[断点续操作](#15)。  
（3）java.lang.OutOfMemoryError: unable to create new native thread   
与（1）类似，内存溢出导致无法继续创建更多线程或对象，降低线程数 threads 重新运行。  
（4）java.lang.OutOfMemoryError: Java heap space   
运行过程中 jvm 堆内存（一般默认为系统内存的 1/4）不足，降低线程数 threads 或者 unit-len 重新运行，如果已运行较长时间可采用[断点续操作](#15)。  
（5）如果您对 java 熟悉的话，可以加上一些调优参数，如 -Xmx 等来调整运行时内存等。  

# 程序日志
qsuits-java 使用 slf4j+log4j2 来记录运行日志，日志产生在当前路径的 logs 目录下，说明如下：  
1、数据源位置记录信息 =\> procedure.log，记录行格式为 json，数据源读取位置打点数据，每一行都是一次数据源位置记录，最后一行即为最后记录下的位置信息，如果信息为 `{}` 表明程序运行完整，没有断点需要再次运行，如果信息中包含具体的字符串，说明这是程序留下的断点，则该行信息可以取出作为断点操作的配置内容，具体参考：[断点操作](#15)  
...
**注意：  
（1）如果是系统宕机、断电或者强制关机或者进程强行 kill 等情况，无法得到输出的断点文件提示，因此只能通过[<位置记录日志>](#14)来查看最后的断点信息，在 8.2.1 版本以上设置了 log 参数可用于启用日志记录的断点，即取出运行路径下 logs 目录中的 procedure[x].log 日志，将该日志文件设置为 `-log=<procedure[x].log's path>` 再运行可完成断点续操作。  
（2）如果原任务包含 process 过程（只有数据源读取不包含 process 操作可以不考虑该问题），执行断点操作时，由于断点日志粒度按照 unit-len 来记录的， 当 unit-len 比较大（默认一般都是 1000 以上，甚至是 10000）时，可能存在记录的断点比实际 process 的进度要滞后很多条记录，因此对于一些不希望存在数 据重复执行的 process，如 qupload/syncupload/mirror/fetch/asyncfetch 等，数据重复执行会影响效率和增加流量消耗，那么建议操作断点时设置一些 check 参数，如 check=stat（参考：[过滤和检查](https://github.com/NigelWu95/qiniu-suits-java/blob/master/docs/datamigration.md#%E8%BF%87%E6%BB%A4%E5%92%8C%E6%A3%80%E6%9F%A5)），或者对于其他操作有这个需求的话可以自行对 process 的[结果输出文件](https://github.com/NigelWu95/qiniu-suits-java#%E5%85%B3%E4%BA%8E%E6%8C%81%E4%B9%85%E5%8C%96%E6%96%87%E4%BB%B6%E5%90%8D)进行检查，查看执行到的数据行位置并对断点设置的 json 文件进行调整。**  
...
暂停（Ctrl + Z 命令）：  
![](http://devtools.qiniu.com/qsuits_ctrlz_to_pause.jpg)  
恢复（fg 命令，注意该命令需要在暂停时的同个 terminal 下执行，同时建议不要做路径切换）：  
![](http://devtools.qiniu.com/qsuits_fg_to_continue.jpg)  
